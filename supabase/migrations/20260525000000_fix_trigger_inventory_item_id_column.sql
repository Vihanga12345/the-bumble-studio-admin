-- =============================================================================
-- Fix: handle_order_stock_and_finance trigger references soi.product_id
-- but the column in sales_order_items is named inventory_item_id
-- This caused "column soi.product_id does not exist" error when updating
-- order status to Delivered from the Manual Sales Orders page.
-- =============================================================================
...
BEGIN
    ...
    THEN
        -- Use inventory_item_id (the actual column name in sales_order_items).
        -- Also handle product_id fallback in case some rows use that column.
    ...
    ELSE
        -- Intermediate workflow steps — do not touch financial transactions
        RETURN NEW;
    END IF;
